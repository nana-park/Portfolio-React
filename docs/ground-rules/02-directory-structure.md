...
같은 역할의 구현이 두 번째 등장하면 `공통화 후보`로 메모합니다. 세 번째 등장하면 다음 페이지 완료 검토의 필수 결정 대상으로 표시합니다. 개발 도중에는 공통화 비교나 리팩터링을 시작하지 않습니다.

후보는 [`docs/design-system/reuse-candidates.md`](../design-system/reuse-candidates.md)에 짧게 기록합니다.

## 공통화 검토 시점

### 개발 중과 일반 PR

- 비슷한 구현을 발견하면 후보 메모만 남깁니다.
- `feature → beta` PR에서는 공통화 결정을 하지 않습니다.
- PR에서는 새 후보가 누락 없이 기록됐는지만 확인합니다.
- 공통 컴포넌트를 직접 변경한 PR만 영향받는 페이지를 대상으로 회귀 테스트합니다. 이는 공통화 검토가 아니라 기존 화면 보호를 위한 테스트입니다.

### 페이지 완료 점검

페이지를 완료 버전으로 표시하기 전에 약 15분 동안 처음으로 공통화 후보를 비교합니다.

- 현재 페이지와 이미 완료된 페이지의 후보 비교
